...
from tests.conftest import CONTRACTS, REFERENCE, REPO_ROOT
...
def test_every_rendered_compose_variable_carries_a_default_or_refuses():
    rendered = dict(render_control_plane(load(REFERENCE, contracts=CONTRACTS)))[COMPOSE_FILE]
...
def test_the_renderer_produces_exactly_its_declared_files():
    produced = {path for path, _ in render_control_plane(load(REFERENCE, contracts=CONTRACTS))}
