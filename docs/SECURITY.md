...
The receipt additionally records the deployment authorization, the private
inventory and the exact image digests, all three by identity rather than by
value. That is what makes "every running byte explained" checkable: a reader
can re-derive each digest and compare, without any of the three documents
disclosing what they resolved to.
...
Since ADR-0006 the check has a document to happen in.
`contracts/deployment-authorization.schema.json` binds the release revision and
bundle digest, the private inventory's identity, the rendered digest, every
image digest, the logical target host, and a named approver with a rationale,
in one instance written BEFORE the promotion. Any three of those agreeing
proves nothing about the fourth, which is why they are bound together rather
than checked one at a time. The instance is private — it names a host and an
approver — and the public receipt records only its digest.
